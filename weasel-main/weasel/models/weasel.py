...
class Weasel(LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        # Convert any float64 values to float32
        converted_stats = {}
        for key, value in stats.items():
            if isinstance(value, torch.Tensor):
                # Convert any tensor to float32
                converted_stats[key] = value.to(torch.float32)
            else:
                # For non-tensor values
                converted_stats[key] = value
        
        self.log_dict(converted_stats, prog_bar=True)
    ...
    def configure_optimizers(self):
        ...
        if self.hparams.optim_end_model is None:
            ...
            # Ensure optim_end_model is a dictionary before passing to get_optimizer
            optim_end_model_dict = self.hparams.optim_end_model
# start of change
            if isinstance(optim_end_model_dict, list):  # If it's a list, convert to dictionary
                parsed_dict = {}
                optimizer_type = None
                
                for item in optim_end_model_dict:
                    if '=' in item:
                        key, value = item.split('=')
                        # Skip _target key as it's not valid for optimizer constructor
                        if key == '_target_' or key == '_target':
                            continue
                        # Try to convert to float, but if it fails, keep as string
                        try:
                            parsed_dict[key] = float(value)
                        except ValueError:
                            parsed_dict[key] = value
                    else:
                        # This might be the optimizer name
                        optimizer_type = item
                
                # Store the optimizer type if needed elsewhere
                if optimizer_type is not None:
                    parsed_dict['name'] = optimizer_type.split('.')[-1].lower()  # Extract 'Adam' from 'torch.optim.Adam'
                
                optim_end_model_dict = parsed_dict

            # end_optimizer = get_optimizer(self.end_model, **optim_end_model_dict)
            end_optimizer = get_optimizer(self.end_model, **vars(optim_end_model_dict))
# end of change
